...
def cholesky(A):
    ...
    for i in range(n):
        for j in range(i + 1):
            if i == j:
                L[i, j] = math.sqrt(matrix[i, i] - np.sum(L[i, :j]**2))
            else:
...
def fixed_point(func, x_0, tol=1e-6, iteration_max=100, derivative=None):
    x_n1 = x_0
    i = 0
    ...
    for iteration in range(iteration_max):
        ...
        if abs(x_n2 - x_n1) < tol:
            sol = x_n2
            break  # Exit the loop once the solution is within tolerance
        x_n1 = x_n2
        i += 1
...
def semi_imp_euler_secant(dydx, y0, x0, xf, st):
    x = []
    y = []
    x.append(x0)
    y.append(y0)
    ...
    for i in range(n):
        ...
        # Implicit part using secant method
        implicit_eq = lambda y_new: y_new - y[i] - st * dydx(y_new, x[i+1])
        y_new_implicit = secant_method(implicit_eq, y[i], y[i]+st, 0.000001)

        # Explicit part
        y_new_explicit = dydx(y_new_implicit, x[i+1])

        y.append(y_new_explicit)
